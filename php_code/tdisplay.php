...
function csashow()
{
    ...
    if ($rtype =='i')
        {
        $dsql = "select pvalue,  LPAD( cast(pdisplay as unsigned ), 4, '0'),pdesc from csadata.picklist where ptype ='" .$ptype ."' order by 1";
        }

    else
        {
        $dsql = "select pdisplay, pdisplay,pdesc from csadata.picklist where ptype ='" .$ptype ."' order by 1"; 
        }
