...
function csashow()
{
    ...
    if ($rtype =='i')
        {
        $dsql = "select pvalue,  LPAD( cast(pvalue as unsigned ), 4, '0'),pdesc from csadata.picklist where ptype ='" .$ptype ."' and ptype1='" .$ptype1 . "' order by 1";
        }

    else
        {
        $dsql = "select pvalue, pdisplay,pdesc from csadata.picklist where ptype ='" .$ptype ."' and ptype1='" .$ptype1 . "' order by 1"; 
        }
