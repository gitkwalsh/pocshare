...
def esxinfo(href):
    sshvo = server_service.getServerHardwareVO(href)
    ssvo = server_service.getServerVO(href)
    netint= sshvo.getInterfaces()
    print "vmHost=%s" % ssvo.getName() 
    print "ESXIP=%s" % netint[0].getIpAddress()
    ...
    for h_vm in h_vms:
        if  h_vm.getName() == vm_name:
            ...
            if len(ca_svr_refs) > 0:
                    ...
                    server_service.decommission(ca_svr_refs[0])
                    server_service.remove(ca_svr_refs[0])
                    v_tries=16
